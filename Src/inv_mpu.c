...
#include "stm32f1xx.h"
#include "i2c.h"
#define i2c_write(a, b, c, d)   I2cWrite(a, b, d, c)
#define i2c_read(a, b, c, d)    I2cRead(a, b, d, c)
#define delay_ms    I2cDelay
#define get_ms      I2cGetMs
...
void SetHwAddr(uint8_t deviceAddr){
    hw.addr = deviceAddr;
}
...
int mpu_init(struct int_param_s *int_param)
{
   ...
   st.chip_cfg.accel_half = 0;
    ...
    printf("Set gyro full range\n");
    if (mpu_set_gyro_fsr(2000))
        return -1;
    printf("Set accel full range\n");
    if (mpu_set_accel_fsr(2))
        return -1;
    printf("Set lowpass filter\n");
    if (mpu_set_lpf(42))
        return -1;
    printf("Set sample rate\n");
    if (mpu_set_sample_rate(50))
        return -1;
    printf("configure fifo\n");
